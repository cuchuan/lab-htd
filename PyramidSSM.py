...
from Selective_scan_interface import selective_scan_fn, HAS_SELECTIVE_SCAN_CUDA
...
def print_backend_status(device=None):
    '''
    Print which compute backend is actually used, so it is clear at a glance
    whether the fast CUDA kernels or the pure-PyTorch fallback are running.
    '''
    if device is None:
        device = "cuda:0" if torch.cuda.is_available() else "cpu"
    scan_status = "ON(cuda kernel)" if HAS_SELECTIVE_SCAN_CUDA else "OFF(fallback)"
    conv_status = "ON(cuda kernel)" if HAS_CAUSAL_CONV1D else "OFF(fallback)"
    print(
        "[HTD-Mamba] device=%s | selective_scan_cuda=%s | causal_conv1d=%s"
        % (device, scan_status, conv_status)
    )
...
class SSM_Manipulation(nn.Module):
    ...
    def __init__(self, d_model, state_size, d_inner, d_conv, conv_bias=True, use_casual1D=None, adjust=True):
        ...
        self.act = nn.SiLU()
        # Auto-select the CUDA kernel only when it is available (e.g. Linux + CUDA build).
        # On Windows/CPU it falls back to an equivalent nn.Conv1d + SiLU implementation.
        if use_casual1D is None:
            use_casual1D = HAS_CAUSAL_CONV1D
        self.use_casual1D = use_casual1D and HAS_CAUSAL_CONV1D
        if adjust:
...
class S6(nn.Module):
    def __init__(self, d_model, d_state=16, d_inner=128, dt_rank="auto", dt_min=0.001,
                 ...
                 dt_init="random",
                 dt_scale=1.0,
                 dt_init_floor=1e-4, use_scan_cuda=None):
        ...
        # Auto-select the CUDA selective-scan kernel only when it is available.
        # On Windows/CPU it falls back to the equivalent pure-PyTorch scan loop.
        if use_scan_cuda is None:
            use_scan_cuda = HAS_SELECTIVE_SCAN_CUDA
        self.use_scan_cuda = use_scan_cuda and HAS_SELECTIVE_SCAN_CUDA
...
if __name__ == '__main__':
    ...
    ssm = S6(d_model=16, d_state=16, d_inner=32).to(device)
